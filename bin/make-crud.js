...
(async () => {
    ...
    for (let e of crudSetup) {
        ...
        const createCommand = new Command(`Create${e}Command`, `../server/Boundary/Commands/${e}/Create${e}Command.ts`, e, entity.values);
        ...
        }, `const q = new Query();\n        q.setTableName('${e.toLowerCase()}');\n        return await q.limit(command.getLimit())\n            .offset(command.getOffset())\n            .get();`);

        listCommandHandler.addImport('Query', '../../../Entities/Query');
        ...
        const params = entity.values.reduce((prev, current) => `${prev.split(":")[0]}, ${current.split(":")[0]}`);
        const controller = new Controller(e + "Controller", `../server/Application/Controllers/${e}Controller.ts`, [
            ...
            {name: "create", body: `const { ${params} } = req.body;\n        const commands = new Create${e}Command(${params});\n        const data = await bus.execute({ commands });\n        return data;`},
            {name: "read", body: `const { id } = req.body;\n        const commands = new Read${e}Command(id);\n        const data = await bus.execute({ commands });\n        return data;`},
